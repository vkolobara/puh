...
absDiff = map (abs . uncurry (-)) . helper
  where helper xs = zip xs (tail xs)
...
studentsPassed :: [(s, Int)] -> [s]
studentsPassed xs = let
  maxScore = maximum $ map snd xs
  filterFn score = fromIntegral score >  0.5 * fromIntegral maxScore
  in map fst $ filter (filterFn . snd) xs
...
sortPairs = sortBy (\x y -> compare (snd x) (snd y))
...
elem' :: Eq a => a -> [a] -> Bool
elem' x = foldr (\a acc -> acc || (a == x)) False
...
nubRuns :: Eq a => [a] -> [a]
nubRuns = foldr (\x xs -> if ((not . null) xs && x == head xs) then xs else x:xs) []
...
sumEven' :: [Integer] -> Integer
sumEven' = let
  sumIfEven (i, xs) x = if i `mod` 2 == 0 then (i + 1, xs + x) else (i + 1, xs)
  in snd . foldl sumIfEven (0, 0)
...
maxUnzip = foldl (\(x, y) (mx, my) -> (max mx x, max my y)) (0, 0)
...
translate :: Point -> Shape2 -> Shape2  
translate (Point x' y') (Circle2 (Point x y) r) = let
    p = Point (x + x') (y + y')
 in Circle2 p r
...
inShape :: Shape2 -> Point -> Bool
inShape (Circle2 (Point x y) r) (Point a b) = (x - a)^2 + (y - b)^2 <= r^2
inShape (Rectangle2 (Point x1 y1) (Point x2 y2)) (Point a b) = 
  x1 <= a && a <= x2 && y1 <= b && b <= y2

inShapes :: [Shape2] -> Point -> Bool
inShapes xs p = or $ map (uncurry inShape) $ zip xs (repeat p)
...
horsePower (Car _ a) = a
horsePower (Truck _ a) = a
horsePower (Motorcycle _ a) = a
...
improveStudent :: Student -> Student
improveStudent s = s {avgGrade = min 5.0 (1 + avgGrade s)}
...
--avgGradePerLevels :: [Student] -> (Double, Double, Double)
avgGradePerLevels xs = let
  avgList l = (sum l) / (genericLength l)
  avgF t xs = avgList $ map (avgGrade) (filter ((==t) . level) xs)
  in (avgF Bachelor xs, avgF Master xs, avgF PhD xs)
...
rankedStudents :: Level -> [Student] -> [String]
rankedStudents l = map studentId 
  . sortBy (\x y -> compare (avgGrade y) (avgGrade x)) 
  . filter ((==l) . level)
...
addStudent :: Student -> [Student] -> [Student]
addStudent x xs = if (studentId x) `elem` (map studentId xs) then error "Already exists!" else x:xs
...
toTriplet :: MyTriplet a b c -> (a, b, c)
toTriplet triplet = (first triplet, second triplet, third triplet)
...
emplSalary :: Maybe Double -> Double
emplSalary Nothing = 0
emplSalary (Just n)  = n

totalSalaries :: [Employee] -> Double
totalSalaries = sum . map (emplSalary . salary)
...
addStudent2 :: Student -> [Student] -> Maybe [Student]
addStudent2 x xs = if (studentId x) `elem` (map studentId xs) 
  then Nothing
  else Just (x:xs)

addStudent3 :: Student -> [Student] -> Either String [Student]
addStudent3 x xs = if (studentId x) `elem` (map studentId xs) 
  then Left "Already exists!"
  else Right (x:xs)
