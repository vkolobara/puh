...
> compareRelativeAge :: (Ageing a, Ageing b) => a -> b -> Ordering
> compareRelativeAge x y = compare (currentAge x `div` maxAge x) (currentAge y `div` maxAge y)
...
>   takeSome 0 t              = []
>   takeSome n Null           = []
>   takeSome n (Node val l r) = takeL ++ takeVal ++ takeR
>     where takeL   = takeSome n l
>           takeVal = if n > length takeL then [val] else []
>           takeR   = if n-1 > length takeL then takeSome (n - 1 - length takeL) r else []
...
>   headOf (Node val _ _ ) = val
>   headOf Null            = error "empty tree"
>   headOff (Node _ l _)   = l
>   headOff Null           = error "empty tree"

> instance Headed Maybe where
>   headOf (Just a)  = a
>   headOf Nothing   = error "empty maybe"
>   headOff (Just _) = Nothing
>   headOff Nothing  = error "empty maybe"
...
>   fmap f (RoseTree val subtrees) = RoseTree (f val) $ map (fmap f) subtrees
